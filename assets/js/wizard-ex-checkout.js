document.addEventListener("DOMContentLoaded", function (e) {
  var t = parseInt(window.Helpers.getCssVar("gray-200", !0).slice(1, 3), 16),
    r = parseInt(window.Helpers.getCssVar("gray-200", !0).slice(3, 5), 16),
    a = parseInt(window.Helpers.getCssVar("gray-200", !0).slice(5, 7), 16),
    l = document.querySelectorAll(".read-only-ratings");
  let n =
  ...
  (() => {
    ...
    let t = document.querySelector(".credit-card-mask"),
      r = document.querySelector(".expiry-date-mask"),
      ...
      (t.addEventListener("input", (e) => {
        ...
        document.querySelector(".card-type").innerHTML =
          e && "unknown" !== e && "general" !== e
            ? `<img src="${assetsPath}img/icons/payments/${e}-cc.png" height="19"/>`
            : "";
      }),
      registerCursorTracker({ input: t, delimiter: " " })),
      r &&
        (r.addEventListener("input", (e) => {
          r.value = formatDate(e.target.value, {
            delimiter: "/",
            datePattern: ["m", "y"],
          });
        }),
    ...
    var e = document.querySelector("#wizard-checkout"),
      l = [].slice.call(e.querySelectorAll(".btn-next")),
      n = e.querySelector(".btn-submit");
    if (null !== e) {
