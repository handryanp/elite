(() => {
  ...
  a.addEventListener("hidden.bs.modal", function () {
    ...
    document.querySelectorAll(".carousel").forEach((r) => {
      r.addEventListener("slide.bs.carousel", (a) => {
        a.relatedTarget
          ? requestAnimationFrame(() => {
              a.relatedTarget.offsetHeight;
              var e = Math.max(
                  a.relatedTarget.offsetHeight,
                  a.relatedTarget.scrollHeight,
                  a.relatedTarget.getBoundingClientRect().height
                ),
                t = r.querySelector(".active.carousel-item").parentElement;
              0 < e &&
                t &&
                t.animate(
                  [{ height: t.offsetHeight + "px" }, { height: e + "px" }],
