(() => {
  ...
  if (
    (e && e.flatpickr({ mode: "range" }),
    window.Helpers.initCustomOptionCheck(),
    null !== (e = document.querySelector("#wizard-create-deal")))
  ) {
    ...
    let o = new Stepper(e, { linear: !0 }),
      t = FormValidation.formValidation(r, {
        fields: {
          dealAmount: {
            validators: {
              notEmpty: { message: "Please enter amount" },
              numeric: { message: "The amount must be a number" },
            },
          },
          dealRegion: {
            validators: { notEmpty: { message: "Please select region" } },
          },
        },
        plugins: {
    ...
    e.length &&
      ...
      e
        ...
        .on("change", function () {
          t.revalidateField("dealRegion");
        }));
    let a = FormValidation.formValidation(s, {
      fields: {
        dealTitle: {
          validators: { notEmpty: { message: "Please enter deal title" } },
        },
        dealCode: {
          validators: {
            notEmpty: { message: "Please enter deal code" },
            stringLength: {
              min: 4,
              max: 10,
              message:
